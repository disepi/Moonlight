...
public class SpeedC extends Check {
    // Constructor
    public SpeedC() {
        super("SpeedC", "Invalid strafe movement", 8);
    }

    public void check(MovePlayerPacket e, PlayerData d, Player p) {
        if (!d.onGround && d.offGroundTicks > 1 && d.currentSpeed > 0.25) {
            ...
            if (!(velZ <= 0.0f && velZ > predictedVelZ || velZ >= 0.0f && velZ < predictedVelZ || velX <= 0.0f && velX > predictedVelX || velX >= 0.0f && velX < predictedVelX)) {
                fail(p, "expectedX=" + predictedVelX + ", expectedZ=" + predictedVelZ + ", x=" + velX + ", z=" + velZ);
                lagback(p, d);
            }
        }
    }
